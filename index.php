...
if(isset($_POST['register'])){
	...
	if ($password !== $repassword) {
		...
		$url = 'http://'.$_SERVER['SERVER_NAME'].'/mail-login/verify.php?id='.$lastId.'&token='.$token;                                // Set email format to HTML
		
		$output = '<div>Thanks for registering with localhost. Please click this link to complete this registation <br> <a href="'.$url.'">Click Here To Verify</a></div>';
		...
		if ($result == true) {
			...
			$mail->Username = '31face8c1ea4f2';                 		// SMTP username
			$mail->Password = 'ccce00262094ae';                           // SMTP password
